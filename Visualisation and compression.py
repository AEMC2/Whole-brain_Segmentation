...
def case2(root):
    ...
    root.withdraw()
    # img_array = []
    csv_path = filedialog.askopenfilename(title='请选择一个csv路径文件')
    ...
    output_video = cv2.VideoWriter(str(csv_path[:-4]) + 'track_video.avi', cv2.VideoWriter_fourcc(*'MJPG'), 10,
                                   (W, H))
...
def get_first_frame(video_path):
    # get patch edge(the first frame and the enhance frame)
    data_name = os.path.basename(video_path)
    print(r'Z:\A+H\000' + str(data_name[:-4]) + 'gray.jpeg')
    video = cv2.VideoCapture(video_path)
    ok, frame = video.read()
    if ok:
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        enhance_gray = np.interp(gray, (200, 255), (0, 255)).astype(np.uint8)
        data_name = os.path.basename(video_path)
        # cv2.imwrite(r'Z:\A+H\000\\'+str(data_name[:-4]) + 'gray.jpeg',gray)
        cv2.imwrite(r'Z:\A+H\000\\'+str(data_name[:-4]) + 'enhance_gray.jpeg',enhance_gray)

def case3(video_path):
...
def compress_vid(root):
    ...
    results = [pool.apply_async(case3, args=(os.path.join(path, files_path),)) for files_path in files]
